...
Concurrency flow (happy path)
1) Controller extracts sub; pass to IJitProvisioningService.ProvisionAsync(sub, request, ct).
2) Service calls repo.GetBySubAsync; if found return (Created=false).
3) Not found: coordinator.RunSingleFlightAsync(sub, createFunc, timeout). The first caller executes createFunc which:
